...
try:
    ...
    if gemini_key and _genai_sdk is not None:
        ...
        print(f"[ConsumerShield] Gemini enabled ({GEMINI_MODEL_NAME}) - via OpenAI SDK")
    elif gemini_key and _genai_sdk is None:
        print("[ConsumerShield] GEMINI_API_KEY set but openai SDK not installed — using rule-based insights")
    else:
...
async def make_ai_insight(
    ...
    async def get_gemini_insight():
        ...
        if screenshot_data_url and isinstance(screenshot_data_url, str) and screenshot_data_url.startswith("data:"):
            contents.append({
                "type": "image_url",
            ...
            try:
                response = await asyncio.wait_for(
                    _gemini_client.chat.completions.create(
                        model=model,
                        messages=[
                            {"role": "system", "content": FORENSIC_AUDITOR_SYSTEM_INSTRUCTION},
                ...
                raw = getattr(getattr(getattr(response, "choices", [None])[0], "message", None), "content", None)
...
async def analyze_complete(req: CompleteRequest, db: Session = Depends(get_db)):
    ...
    # Fallback heuristic for live/demo app text when model signals are sparse.
    raw_text = str(getattr(req.manipulation_data, "raw_text", "") or "")
    base_score = 0.0
    if "OFF" in raw_text:
        base_score += 3.0
    if "RECOMMENDED" in raw_text:
        base_score += 2.0
    if "Free" in raw_text:
        base_score += 1.0

    o_risk = float(min(10.0, max(o_risk, base_score)))
